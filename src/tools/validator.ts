 ...
 */
function checkEmailField (email: any, req: express.Request, res: express.Response): boolean {
    if (email === undefined || email === '') {
        new ErrLog(res.locals.lang.error.email.required, Log.CODE.BAD_REQUEST, 'email').sendTo(res);
        return false;
    }
    if (typeof email !== 'string') {
        new ErrLog(res.locals.lang.error.email.type, Log.CODE.BAD_REQUEST, 'email').sendTo(res);
        return false;
    }
    if (!IsEmail.validate(email)) {
        new ErrLog(res.locals.lang.error.email.invalid, Log.CODE.EXPECTATION_FAILED, 'email').sendTo(res);
        return false;
    }
    if (email.length < 6 || email.length > 255) {
        new ErrLog(res.locals.lang.error.email.length, Log.CODE.EXPECTATION_FAILED, 'email').sendTo(res);
        return false;
    }
    return true;
}
...
function checkIdField (id: any, req: express.Request, res: express.Response): boolean {
    if (id === undefined || id === '') {
        new ErrLog(res.locals.lang.error.id.required, Log.CODE.BAD_REQUEST, 'password').sendTo(res);
        return false;
    }
    if (typeof id !== 'number') {
        new ErrLog(res.locals.lang.error.id.type, Log.CODE.BAD_REQUEST, 'password').sendTo(res);
        return false;
    }
...
function checkBooleanField (value: any, req: express.Request, res: express.Response): boolean {
    ...
    if (typeof value !== 'boolean') {
        new ErrLog(res.locals.error.boolean.type, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    return true;
}
...
function checkDateField (date: any, req: express.Request, res: express.Response): boolean {
    if (date === undefined || date === '') {
        new ErrLog(res.locals.error.date.required, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    if (isNaN(new Date(date).getTime())) {
        new ErrLog(res.locals.error.date.invalid, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    return true;
}
...
function checkPhoneField (phone: any, req: express.Request, res: express.Response): boolean {
    if (phone === undefined || phone === '') {
        new ErrLog(res.locals.error.phone.required, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    if (typeof phone !== 'string') {
        new ErrLog(res.locals.error.phone.type, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    const num = phone.replace(/(\.|\s|-)/g, '').trim();
    if (num.match(/^((00[0-9]{2})?0[0-9][0-9]{8}|\+[0-9]{11,12})$/) === null) {
        new ErrLog(res.locals.error.phone.invalid, Log.CODE.EXPECTATION_FAILED).sendTo(res);
        return false;
    }
    return true;
}
...
function checkStringField (value: any, req: express.Request, res: express.Response): boolean {
    if (value === undefined || value === '' || value === null) {
        new ErrLog(res.locals.error.string.required, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    if (typeof value !== 'string') {
        new ErrLog(res.locals.error.string.type, Log.CODE.BAD_REQUEST).sendTo(res);
        return false;
    }
    return true;
}
...
function checkNameField (value: any, req: express.Request, res: express.Response): boolean {
    ...
    if (value.match(/^[a-zA-ZÀ-ÿ- ]+$/) === null) {
        new ErrLog(res.locals.error.name.invalid, Log.CODE.EXPECTATION_FAILED).sendTo(res);
        return false;
    }
    return true;
}
